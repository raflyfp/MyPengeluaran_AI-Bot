...
<article
    x-data="{ visible: true }"
    class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#16B69C] via-[#087B69] to-[#093C5D] p-6 text-white shadow-[0_22px_44px_rgba(9,60,93,0.22)] transition duration-200 active:scale-[0.99]"
>
    ...
    <div class="relative">
        <div class="mb-3 flex items-center justify-between">
            <h2 class="text-xs font-bold uppercase tracking-[0.14em] text-white/78">{{ $label }}</h2>
            <button
                type="button"
                class="rounded-full p-2 text-white/80 transition hover:bg-white/10 active:scale-95"
        ...
        <p class="mb-5 text-[2.55rem] font-extrabold leading-none tracking-normal sm:text-5xl">
            <span x-show="visible">{{ $amount }}</span>
            <span x-show="! visible">Rp ********</span>
        </p>

        <div class="flex items-center gap-3 text-sm font-semibold">
            <span class="inline-flex items-center gap-1 rounded-full bg-white/20 px-3 py-1.5 text-white shadow-inner backdrop-blur">
                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                    <path d="M12 19V5M6 11l6-6 6 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
                {{ $change }}
            </span>
            <span class="text-white/78">{{ $caption }}</span>
        </div>
    </div>
</article>
